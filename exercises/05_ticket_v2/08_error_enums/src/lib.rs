...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        Ok(result) => result,
        Err(TicketNewError::Title { detail }) => panic!("{}", detail),
        Err(TicketNewError::Description { detail: _ }) => {
...
impl Ticket {
    pub fn new(
        ...
        if title.len() > 50 {
            return Err(TicketNewError::Title {
                detail: "Title cannot be longer than 50 bytes".to_string(),
            });
        }
        if description.is_empty() {
            return Err(TicketNewError::Description {
                detail: "Description cannot be empty".to_string(),
            });
        }
        if description.len() > 500 {
            return Err(TicketNewError::Description {
                detail: "Description cannot be longer than 500 bytes".to_string(),
            });
        }
